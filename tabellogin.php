...
if ($op == 'delete' && isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql1 = "DELETE FROM operasi WHERE id_operasi = ?";
...
if (isset($_POST['checkout'])) {
    if (isset($_POST['id_operasi'])) {
        $id = $_POST['id_operasi'];
        
        // Update tanggal_keluar
        $jam_keluar = date('H:i:s'); 
        $sql2 = "UPDATE operasi SET jam_keluar = ? WHERE id_operasi = ?";
        $stmt2 = mysqli_prepare($koneksi, $sql2);
        mysqli_stmt_bind_param($stmt2, 'ss', $jam_keluar, $id);
